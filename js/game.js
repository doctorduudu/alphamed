...
let newGameList = gameList.filter(function(game) {
  return (
    (game.topic === topic1 || game.topic === topic2) &&
    (game.system === system1 ||
      game.system === system2 ||
      game.system === system3) &&
    (game.level === level1 || game.level === level2) &&
    (game.type === type1 || game.type === type2)
  );
});
...
if (newGameList.length === 0) {
  alert(
    "Sorry we do not have any terms that match your selection at the moment, you can contact Us to request for it to be added. Thank You"
  );
...
function createInstance() {
  currentGame = new gameObject(
    newGameList[valueRemoved].name,
    `img/${imageFetcher}-a.jpg`,
    `img/${imageFetcher}-b.jpg`,
    `img/${imageFetcher}-c.jpg`,
    `img/${imageFetcher}-d.jpg`,
    newGameList[valueRemoved].summary,
    newGameList[valueRemoved].etiology,
    newGameList[valueRemoved].symptoms,
    newGameList[valueRemoved].source,
    newGameList[valueRemoved].sourceLink
  );
}
createInstance();
...
function createGame(currentGame) {
  ...
  function checkAns() {
    ...
    progressBarWidth = Math.floor(
      (noOfQuestionsSolved / newGameList.length) * 100
    );
  ...
  function makeCorrection() {
    let correctionPointerHtml = `
            <p class="h5 text-danger">Oops! <i class="fas fa-frown h5 mr-3 text-danger"></i><span class="text-success"> Answer </span><i class="fas fa-hand-point-down h4 ml-3 text-success"></i></p>   
        `;
...
function makegameEndMessage() {
  let gameEndScorePercentage = Math.round(
    (scoreCount / (20 * newGameList.length)) * 100
  );
...
function getSpeed() {
  ...
  if (speedPrecise > Number(localStorage.getItem("bestSpeed"))) {
    ...
    bestSpeed.textContent = `Best Speed: ${localStorage.getItem(
      "bestSpeed"
    )}...(new Best Speed)`;
  } else {
    bestSpeed.textContent = `Best Speed: ${localStorage.getItem(
